...
public class AESUtils {
    ...
    //私钥
    public static final String AES_KEY = "1111222233334444";   //AES固定格式为128/192/256 bits.即：16/24/32bytes。DES固定格式为128bits，即8bytes。
    ...
    public static String encrypt(String cleartext, String aesKey) {
        try {
            if (StringUtils.isEmpty(cleartext)) { //避免传入的加密内容为空造成空指针异常
                return "";
            }
            ...
            //使用jdk中未过时的Base64编码器，超过76个字符无换行 RFC-4648
...
//        return new BASE64Encoder().encode(encryptedData);
    ...
    public static String decrypt(String encrypted, String aesKey) throws Exception {
        byte[] byteMi = Base64Utils.decode(encrypted.getBytes(StandardCharsets.UTF_8));
