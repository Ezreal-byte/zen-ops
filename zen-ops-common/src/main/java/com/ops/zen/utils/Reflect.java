...
public class Reflect {
        ...
        if (true)
            ...
            try {
                ...
                // warnings when running the below on JDK 9+
                    ...
                    result = Lookup.class.getDeclaredConstructor(Class.class);
    ...
    /**
     * 类型type对应的实例，如果type是接口则object和type相同
     * The wrapped object.
     */
     ...
     * @return
     */
    public static <T> T newInstance(Class<? extends T> clazz) throws IllegalAccessException, InstantiationException {
     ...
     */
    public static void set(Object object, Field field, Object fieldValue) throws IllegalAccessException {
     ...
     */
    public Object fieldValue(String name) throws ReflectException {
        try {
            Field field = field0(name);
    ...
    public Map<String, Reflect> fields() {
        ...
        do {
            for (Field field : t.getDeclaredFields()) {
                /*
                如果是对象操作，type != object == true == 1
                Modifier.isStatic(field.getModifiers()) 字段静态时为1，非静态时为0
                也就表示只能获取非静态字段
                ----------
                如果是Class操作type != object == false == 0
                Modifier.isStatic(field.getModifiers()) 字段静态时为1，非静态时为0
                也就表示只能获取静态字段
                 */
                if (type != object ^ Modifier.isStatic(field.getModifiers())) {//0 false 1 true 相同为0 不同为1
    ...
    public Map<String, Object> asMap() {
        ...
        do {
            for (Field field : t.getDeclaredFields()) {
                //异或：不同为1，相同为0
                //静态变量取值：type!=object为false，后面为true，所以条件为true
                //普通成员变量取值：type!=object为true，后面为false，所以条件为true
                if (type != object ^ Modifier.isStatic(field.getModifiers())) {
                    String name = field.getName();

                    if (!result.containsKey(name))
                        result.put(name, fieldValue(name));
                }
            }
     ...
     * @return
     */
    public Map<String, Field> nameFields() {
        return nameFields(false);
    }

    /**
     * 自定义方法，如果onClass 这里只能获取静态字段，如果是on(Object)这里只能获取非静态字段
     *
     * @param fieldAccessable Field是否可以访问
     * @return
     */
    public Map<String, Field> nameFields(boolean fieldAccessable) {
    ...
    /**
     * 不管是onClass还是on都只返回非静态字段
     *
     * @param fieldAccessable Field是否可以访问
     * @return
     */
    public Map<String, Field> nonStaticNameFields(boolean fieldAccessable) {
        Map<String, Field> result = new LinkedHashMap<String, Field>();
        Class<?> t = type();
    ...
    /**
     * 自定义静态方法，获取Class t的所有权限>=private的非静态字段
     *
     * @param t
     * @param fieldAccessable
     * @return
     */
    public static Map<String, Field> nameFields(Class<?> t, boolean fieldAccessable) {
        Map<String, Field> result = new LinkedHashMap<String, Field>();
        do {
            for (Field field : t.getDeclaredFields()) {
                if (!Modifier.isStatic(field.getModifiers())) {
